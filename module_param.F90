...
! Fire Parameters 
!------------------------------------------------------------------
type, public :: fire_param

    !----Live Pool Info----
    !...autotrophic respiration parameters
    real(r4), dimension(:), allocatable :: &  !(npoolpft)
         mfl,   & !Mortality fraction no forest loss low value (0-1)
         mfh      !Mortality fraction no forest loss high value (0-1)

    real(r4), dimension(:), allocatable ::  &  !(ntpool)
         ccl,   & !Pool combustion completeness no forest loss low value (0-1)
         cch      !Pool combustion completeness no forest loss high value (0-1)
...
! Fire Parameters (npft)
  type(fire_param), dimension(:), allocatable :: firecon
