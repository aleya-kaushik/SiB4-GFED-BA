...
do v = 1, sibr_nvar
   ...
   case (128)
    ...
    case (129)
       if (.not. var_exist) then
          status = nf90_inq_varid(ncid, 'loss_transf_lay', varid)
          var_exist = (status .eq. nf90_noerr)
       endif
       ...
       if (var_exist) then
          ...
          status = nf90_get_var(ncid, varid, dvar4d)
          if (status .ne. nf90_noerr) then
               print*,'Error reading loss_transf_lay.'
               stop
          endif
          ...
          do i=1, subcount
             ...
             do l=1,sib%g(i)%g_nlu
                do p=1,npoolpft
                   sib%g(i)%l(l)%poollt%loss_transf_lay(p,:) = dvar4d(gref,l,p,:)
                enddo
             enddo
           enddo
           deallocate(dvar4d)
           ...
           do i=1, subcount
              do l=1,sib%g(i)%g_nlu
                  sib%g(i)%l(l)%poollt%loss_transf_lay(:,:) = sibr_vd(v)
               enddo
           enddo
       endif
   ...
   case (145)
    ...
    case (146)
       if (.not. var_exist) then
          status = nf90_inq_varid(ncid, 'gain_transf_lay', varid)
          var_exist = (status .eq. nf90_noerr)
       endif
       ...
       if (var_exist) then
           ...
           status = nf90_get_var(ncid, varid, dvar4d)
           if (status .ne. nf90_noerr) then
              print*,'Error reading gain_transf_lay.'
              stop
           endif
           ...
           do i=1, subcount
              if (nsibr_flag) then
               ...
               do l=1,sib%g(i)%g_nlu
                  do p=1,npoollu
                     sib%g(i)%l(l)%pooldt%gain_transf_lay(p,:) = &
                              dvar4d(gref,l,p,:)
                   enddo
               enddo
            enddo
           ...
           do i=1, subcount
              do l=1,sib%g(i)%g_nlu
                sib%g(i)%l(l)%pooldt%gain_transf_lay(:,:) = sibr_vd(v)
              enddo
           enddo
       endif
       ...
       if (var_exist) then
           ...
           do i=1, subcount
              if (nsibr_flag) then
               ...
               do l=1,sib%g(i)%g_nlu
                  do p=1,npoollu
                     sib%g(i)%l(l)%pooldt%loss_transf_lay(p,:) = &
                              dvar4d(gref,l,p,:)
                   enddo
               enddo
            enddo
           ...
           do i=1, subcount
              do l=1,sib%g(i)%g_nlu
                sib%g(i)%l(l)%pooldt%loss_transf_lay(:,:) = sibr_vd(v)
              enddo
           enddo
       endif
