...
!...misc variables
integer :: ido, idoc, l, p, s, myl, mylp
...
integer(i4) :: flp, ffrp, fcrp, fwp, fpp
integer(i4) :: fcdbp, fmetlp, fstrlp, fslitp, fslowp, farmp
!real(r8) :: rcpooltest
...
! g    = kg DM m-2 *    %   *   * g(kgDM)^-1
! note: BA units are BA % PFT⁻¹ s⁻¹ (converted in fire_read_ code)

! current code calculation:
! mol C m-2 = mol C m-2  *   fraction  * fraction
! BA units: BA % PFT⁻¹ s⁻¹: converted from /3h to /s in fire_read_ code
...
!print*,'pft_name: ',pft_name(tpnum)
   ...
   curemisc13_dm = dzero
      ...
      !.. note: multiply BA s-1 by dtsib to remove seconds unit

      !.. below is for v14, where global BA totals included pft_area
      !if (tpname(l) .eq. "dbg") myba=sibg%gprogt%ba_dbg*dtsib*tparea(l)
      !if (tpname(l) .eq. "en2") myba=sibg%gprogt%ba_en2*dtsib*tparea(l)
      !if (tpname(l) .eq. "en3") myba=sibg%gprogt%ba_en3*dtsib*tparea(l)     
      !if (tpname(l) .eq. "dnf") myba=sibg%gprogt%ba_dnf*dtsib*tparea(l)
      !if (tpname(l) .eq. "eb1") myba=sibg%gprogt%ba_eb1*dtsib*tparea(l)  
      !if (tpname(l) .eq. "eb2") myba=sibg%gprogt%ba_eb2*dtsib*tparea(l)  
      !if (tpname(l) .eq. "db1") myba=sibg%gprogt%ba_db1*dtsib*tparea(l)  
      !if (tpname(l) .eq. "db2") myba=sibg%gprogt%ba_db2*dtsib*tparea(l)  
      !if (tpname(l) .eq. "db3") myba=sibg%gprogt%ba_db3*dtsib*tparea(l)  
      !if (tpname(l) .eq. "shb") myba=sibg%gprogt%ba_shb*dtsib*tparea(l)  
      !if (tpname(l) .eq. "sha") myba=sibg%gprogt%ba_sha*dtsib*tparea(l)  
      !if (tpname(l) .eq. "c3a") myba=sibg%gprogt%ba_c3a*dtsib*tparea(l)  
      !if (tpname(l) .eq. "c3g") myba=sibg%gprogt%ba_c3g*dtsib*tparea(l)  
      !if (tpname(l) .eq. "c4g") myba=sibg%gprogt%ba_c4g*dtsib*tparea(l)  
      !if (tpname(l) .eq. "c3c") myba=sibg%gprogt%ba_c3c*dtsib*tparea(l)  
      !if (tpname(l) .eq. "c4c") myba=sibg%gprogt%ba_c4c*dtsib*tparea(l)  
      !if (tpname(l) .eq. "mze") myba=sibg%gprogt%ba_mze*dtsib*tparea(l)  
      !if (tpname(l) .eq. "soy") myba=sibg%gprogt%ba_soy*dtsib*tparea(l)  
      !if (tpname(l) .eq. "wwt") myba=sibg%gprogt%ba_wwt*dtsib*tparea(l)  
      
      !v15.. without pft_area
      if (tpname(l) .eq. "dbg") myba=sibg%gprogt%ba_dbg*dtsib
      if (tpname(l) .eq. "en2") myba=sibg%gprogt%ba_en2*dtsib
      if (tpname(l) .eq. "en3") myba=sibg%gprogt%ba_en3*dtsib     
      if (tpname(l) .eq. "dnf") myba=sibg%gprogt%ba_dnf*dtsib
      if (tpname(l) .eq. "eb1") myba=sibg%gprogt%ba_eb1*dtsib  
      if (tpname(l) .eq. "eb2") myba=sibg%gprogt%ba_eb2*dtsib  
      if (tpname(l) .eq. "db1") myba=sibg%gprogt%ba_db1*dtsib  
      if (tpname(l) .eq. "db2") myba=sibg%gprogt%ba_db2*dtsib  
      if (tpname(l) .eq. "db3") myba=sibg%gprogt%ba_db3*dtsib  
      if (tpname(l) .eq. "shb") myba=sibg%gprogt%ba_shb*dtsib  
      if (tpname(l) .eq. "sha") myba=sibg%gprogt%ba_sha*dtsib  
      if (tpname(l) .eq. "c3a") myba=sibg%gprogt%ba_c3a*dtsib  
      if (tpname(l) .eq. "c3g") myba=sibg%gprogt%ba_c3g*dtsib  
      if (tpname(l) .eq. "c4g") myba=sibg%gprogt%ba_c4g*dtsib  
      if (tpname(l) .eq. "c3c") myba=sibg%gprogt%ba_c3c*dtsib  
      if (tpname(l) .eq. "c4c") myba=sibg%gprogt%ba_c4c*dtsib  
      if (tpname(l) .eq. "mze") myba=sibg%gprogt%ba_mze*dtsib  
      if (tpname(l) .eq. "soy") myba=sibg%gprogt%ba_soy*dtsib  
      if (tpname(l) .eq. "wwt") myba=sibg%gprogt%ba_wwt*dtsib                                                           
   !print*,'myl: ',myl
   !print*,'mylp: ',mylp
   !print*,'pftname(l): ',tpname(l)
   !print*,'pftname(l), MFL:',tpname(l),firecon(mylp)%mfl(flp),firecon(mylp)%mfl(frp),firecon(mylp)%mfl(crp),firecon(mylp)%mfl(wp),firecon(mylp)%mfl(pp)
   !print*,'pftname(l), MFH:',tpname(l),firecon(mylp)%mfh(flp),firecon(mylp)%mfh(frp),firecon(mylp)%mfh(crp),firecon(mylp)%mfh(wp),firecon(mylp)%mfh(pp)

!print*,'indexes :',lp,frp,crp,wp,pp,cdbp,metlp,strlp,slitp,slowp,armp

   !print*,'pftname(l), CCL:',tpname(l),firecon(mylp)%ccl(flp),firecon(mylp)%ccl(frp),firecon(mylp)%ccl(crp),firecon(mylp)%ccl(wp),firecon(mylp)%ccl(pp),firecon(mylp)%ccl(cdbp),firecon(mylp)%ccl(metlp),firecon(mylp)%ccl(strlp),firecon(mylp)%ccl(slitp),firecon(mylp)%ccl(slowp),firecon(mylp)%ccl(armp)
   !print*,'pftname(l), CCH:',tpname(l),firecon(mylp)%cch(flp),firecon(mylp)%cch(frp),firecon(mylp)%cch(crp),firecon(mylp)%cch(wp),firecon(mylp)%cch(pp),firecon(mylp)%cch(cdbp),firecon(mylp)%cch(metlp),firecon(mylp)%cch(strlp),firecon(mylp)%cch(slitp),firecon(mylp)%cch(slowp),firecon(mylp)%cch(armp)
      ...
      mort_fract = firecon(mylp)%mfl(flp) + &
                   st_scalar*(firecon(mylp)%mfh(flp) - firecon(mylp)%mfl(flp))
      c_compl = firecon(mylp)%ccl(flp) + &
                   st_scalar*(firecon(mylp)%cch(flp) - firecon(mylp)%ccl(flp))
      ...
      mort_fract = firecon(mylp)%mfl(fwp) + &
                   st_scalar*(firecon(mylp)%mfh(fwp) - firecon(mylp)%mfl(fwp))
      c_compl = firecon(mylp)%ccl(fwp) + &
                   st_scalar*(firecon(mylp)%cch(fwp) - firecon(mylp)%ccl(fwp))
      mymort = myfl * myba * mort_fract
      myemis = mymort * c_compl
      mytrans = mymort * (1.0 - c_compl)
      ...
      !mort_fract = firecon(mylp)%mfl(fmetlp) + &
      !             st_scalar*(firecon(mylp)%mfh(fmetlp) - firecon(mylp)%mfl(fmetlp))
      c_compl = firecon(mylp)%ccl(fmetlp) + &
                   st_scalar*(firecon(mylp)%cch(fmetlp) - firecon(mylp)%ccl(fmetlp))
      mymort = 0.0
      myemis = myfl * myba * c_compl
      mytrans = 0.0

      !print*,'pft_name: ',tpname(l)
      !print*,'myl :',myl
      !print*,'mylp :',mylp
      !print*,'metl myfl:',myfl
      !print*,'metl myba:',myba
      !print*,'metl c_compl:',c_compl
      !print*,'metl myemis: ',myemis
      ...
      !mort_fract = firecon(mylp)%mfl(fstrlp) + &
      !             st_scalar*(firecon(mylp)%mfh(fstrlp) - firecon(mylp)%mfl(fstrlp))
      c_compl = firecon(mylp)%ccl(fstrlp) + &
                   st_scalar*(firecon(mylp)%cch(fstrlp) - firecon(mylp)%ccl(fstrlp))
      mymort = 0.0
      myemis = myfl * myba * c_compl
      mytrans = 0.0
      ...
      !mort_fract = firecon(mylp)%mfl(fcdbp) + &
      !             st_scalar*(firecon(mylp)%mfh(fcdbp) - firecon(mylp)%mfl(fcdbp))
      c_compl = firecon(mylp)%ccl(fcdbp) + &
                   st_scalar*(firecon(mylp)%cch(fcdbp) - firecon(mylp)%ccl(fcdbp))
      mymort = 0.0
      myemis = myfl * myba * c_compl
      mytrans = 0.0
      ...
      mort_fract = firecon(mylp)%mfl(fpp) + &
                   st_scalar*(firecon(mylp)%mfh(fpp) - firecon(mylp)%mfl(fpp))
      c_compl = firecon(mylp)%ccl(fpp) + &
                   st_scalar*(firecon(mylp)%cch(fpp) - firecon(mylp)%ccl(fpp))
      mymort = myfl * myba * mort_fract
      myemis = mymort * c_compl
      mytrans = mymort * (1.0 - c_compl)
      ...
      mort_fract = firecon(mylp)%mfl(ffrp) + &
                   st_scalar*(firecon(mylp)%mfh(ffrp) - firecon(mylp)%mfl(ffrp))
      c_compl = firecon(mylp)%ccl(ffrp) + &
                   st_scalar*(firecon(mylp)%cch(ffrp) - firecon(mylp)%ccl(ffrp))
      mymort = myfl * myba * mort_fract
      myemis = mymort * c_compl
      mytrans = mymort * (1.0 - c_compl)
      ...
      DO s=1,nsoil 
         sibg%l(myl)%poollt%loss_fire_lay(frp,s) = &
              myemis * sibg%l(myl)%poollt%poolpft_flay(frp,s)*dtisib
         sibg%l(myl)%poollt%loss_transf_lay(frp,s) = &
              mytrans * sibg%l(myl)%poollt%poolpft_flay(frp,s)*dtisib
         ...
         sibg%l(myl)%poollt%loss_fire_lay(frpc13,s) = &
              ...
              * sibg%l(myl)%poollt%poolpft_flay(frpc13,s)*dtisib
      ...
      mort_fract = firecon(mylp)%mfl(fcrp) + &
                   st_scalar*(firecon(mylp)%mfh(fcrp) - firecon(mylp)%mfl(fcrp))
      c_compl = firecon(mylp)%ccl(fcrp) + &
                   st_scalar*(firecon(mylp)%cch(fcrp) - firecon(mylp)%ccl(fcrp))
      mymort = myfl * myba * mort_fract
      myemis = mymort * c_compl
      mytrans = mymort * (1.0 - c_compl)
      ...
      DO s=1,nsoil 
         sibg%l(myl)%poollt%loss_fire_lay(crp,s) = &
              myemis * sibg%l(myl)%poollt%poolpft_flay(crp,s)*dtisib
         sibg%l(myl)%poollt%loss_transf_lay(crp,s) = &
              mytrans * sibg%l(myl)%poollt%poolpft_flay(crp,s)*dtisib
         ...
         sibg%l(myl)%poollt%loss_fire_lay(crpc13,s) = &
              ...
              * sibg%l(myl)%poollt%poolpft_flay(crpc13,s)*dtisib
      ...
      !mort_fract = firecon(mylp)%mfl(fslitp) + &
      !             st_scalar*(firecon(mylp)%mfh(fslitp) - firecon(mylp)%mfl(fslitp))
      c_compl = firecon(mylp)%ccl(fslitp) + &
                   st_scalar*(firecon(mylp)%cch(fslitp) - firecon(mylp)%ccl(fslitp))
      mymort = 0.0
      myemis = myfl * myba * c_compl
      mytrans = 0.0
      ...
      DO s=1,nsoil 
        sibg%l(myl)%pooldt%loss_fire_lay(slitp,s) = &
             myemis * sibg%l(myl)%pooldt%poollu_flay(slitp,s)*dtisib
        sibg%l(myl)%pooldt%loss_transf_lay(slitp,s) = &
             mytrans * sibg%l(myl)%pooldt%poollu_flay(slitp,s)*dtisib
        ...
        sibg%l(myl)%pooldt%loss_fire_lay(slitpc13,s) = &
             ...
             * sibg%l(myl)%pooldt%poollu_flay(slitpc13,s)*dtisib
      ...
      !mort_fract = firecon(mylp)%mfl(fslowp) + &
      !             st_scalar*(firecon(mylp)%mfh(fslowp) - firecon(mylp)%mfl(fslowp))
      c_compl = firecon(mylp)%ccl(fslowp) + &
                   st_scalar*(firecon(mylp)%cch(fslowp) - firecon(mylp)%ccl(fslowp))
      mymort = 0.0
      myemis = myfl * myba * c_compl
      mytrans = 0.0
      ...
      DO s=1,nsoil 
         sibg%l(myl)%pooldt%loss_fire_lay(slowp,s) = &
              myemis * sibg%l(myl)%pooldt%poollu_flay(slowp,s)*dtisib
         sibg%l(myl)%pooldt%loss_transf_lay(slowp,s) = &
              mytrans * sibg%l(myl)%pooldt%poollu_flay(slowp,s)*dtisib
         ...
         sibg%l(myl)%pooldt%loss_fire_lay(slowpc13,s) = &
              ...
              * sibg%l(myl)%pooldt%poollu_flay(slowpc13,s)*dtisib
      ...
      !mort_fract = firecon(mylp)%mfl(farmp) + &
      !             st_scalar*(firecon(mylp)%mfh(farmp) - firecon(mylp)%mfl(farmp))
      c_compl = firecon(mylp)%ccl(farmp) + &
                   st_scalar*(firecon(mylp)%cch(farmp) - firecon(mylp)%ccl(farmp))
      mymort = 0.0
      myemis = myfl * myba * c_compl
      mytrans = 0.0
      ...
      DO s=1,nsoil 
          sibg%l(myl)%pooldt%loss_fire_lay(armp,s) = &
               myemis * sibg%l(myl)%pooldt%poollu_flay(armp,s)*dtisib
         sibg%l(myl)%pooldt%loss_transf_lay(armp,s) = &
              mytrans * sibg%l(myl)%pooldt%poollu_flay(armp,s)*dtisib
          ...
          sibg%l(myl)%pooldt%loss_fire_lay(armpc13,s) = &
               ...
               * sibg%l(myl)%pooldt%poollu_flay(armpc13,s)*dtisib
