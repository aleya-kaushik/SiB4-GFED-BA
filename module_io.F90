module module_io
  ...
  use module_sibconst, &
    ...
    character (len=iofnlen) ::     &
        ...
        firep_file,   & ! path to fire param file containing CC/MF
        ...
        firegfed_path,  & ! path to burned area files for fire
    ...
    integer(i4), parameter :: &
        ...
        firepid = 34, &  !file id for fire parameters
    ...
    ! variables in burned area files
    ! burned area distributed according to PFT
    character(len=15), parameter :: &
        ba_dbgname = 'ba_dbg',   &
        ba_en2name = 'ba_en2',   &
        ba_en3name = 'ba_en3',   &
    ...
    character (len=iofnlen) :: firegfed_filename

    ! flags
    logical :: fire_readf   ! read fire data ?
    logical :: fire_switchf ! switch fire data file ?
    logical :: fire_updatef ! update fire data?
    logical :: firegfed_readf ! same as above but for new burned area files
    logical :: firegfed_switchf ! same as above but for new burned area files
    logical :: firegfed_updatef ! same as above but for new burned area files
